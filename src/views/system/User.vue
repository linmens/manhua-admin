	...
	<Auth :authority="['ROLE_ADMIN']" :access="userAccess">
        ...
        row.sex
			...
			<div class="ivu-mt ivu-text-center">
				<Page :total="allUserNum" show-total show-elevator @on-change="pageChange" @on-prev="pageChange"
					@on-next="pageChange" />
			</div>

			<Drawer :styles="drawstyles" title="编辑用户" width="560" :closable="true" v-model="editDrawer">
				<Form>
					<Row :gutter="16">
					...
					<Row :gutter="16">
						...
						<FormItem label="角色分配">
							<Select multiple v-model="editData.roleSet">
								<Option v-for="item in roleList" :key="item.name" :value="item.name">
									<span>{{ item.name }}</span>
				...
				<div class="drawer-footer">
					<Button style="margin-right: 8px" @click="editDrawer = false">取消</Button>
